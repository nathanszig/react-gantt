...
export const weekIndexInTheMonth = (date) => {
  const startOfMonth = moment(date).startOf('month');
  // Calculate the number of days in the first week of the month
  const daysInFirstWeek = moment(date).startOf('week').isoWeekday() - startOfMonth.isoWeekday() + 1;
...
export const numberOfWeeksInMonth = (date) => {
  return weekIndexInTheMonth(moment(date).endOf('month'));
}
